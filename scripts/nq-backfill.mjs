...
const SYMBOL = (process.env.SYMBOL || "NQ").toUpperCase();   // NQ, MNQ, ES, MES
...
const q = await (await fetch(`https://api.supabase.com/v1/projects/${REF}/database/query`, { method: "POST", headers: { Authorization: `Bearer ${mgmt}`, "Content-Type": "application/json" }, body: JSON.stringify({ query: "select symbol, interval, count(*) as bars, min(t) as first, max(t) as last from public.nq_bars group by symbol, interval order by 1, 2" }) })).json();
console.table(q.map((r) => ({ symbol: r.symbol, interval: r.interval, bars: +r.bars, first: r.first.slice(0, 16), last: r.last.slice(0, 16) })));
