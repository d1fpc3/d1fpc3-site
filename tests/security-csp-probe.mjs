...
const TARGET = process.env.TARGET === "admin" ? "admin" : "app";
const ht = readFileSync(new URL(`../echelon/${TARGET}/.htaccess`, import.meta.url), "utf8");
const CSP = (ht.match(/Content-Security-Policy "([^"]+)"/) || [])[1]; if (!CSP) throw new Error("no policy in echelon/" + TARGET + "/.htaccess");
const APP = process.env.APP_URL || `http://127.0.0.1:8124/echelon/${TARGET}/`;
const REF = "cqdignbleethroyxxvzr", SB = `https://${REF}.supabase.co`;
const mgmt = readFileSync(join(homedir(), ".supabase", "access-token"), "utf8").trim();
const keys = await (await fetch(`https://api.supabase.com/v1/projects/${REF}/api-keys?reveal=true`, { headers: { Authorization: `Bearer ${mgmt}` } })).json();
const service = keys.find((k) => k.name === "service_role").api_key, anon = keys.find((k) => k.name === "anon").api_key;
const link = await (await fetch(`${SB}/auth/v1/admin/generate_link`, { method: "POST", headers: { apikey: service, Authorization: `Bearer ${service}`, "Content-Type": "application/json" }, body: JSON.stringify({ type: "magiclink", email: process.env.EMAIL || (TARGET === "admin" ? "d1fpc3@gmail.com" : "appreview@d1fpc3.com") }) })).json();
...
await ctx.addInitScript(([k, v]) => { localStorage.setItem(k, v); localStorage.setItem("echelon-theme", "dark"); localStorage.setItem("echelon-gex-tour", "1"); localStorage.setItem("echelon-admin-tour", "done"); localStorage.setItem("echelon-quotes-off", "1"); localStorage.setItem("echelon-splash-day", new Date().toDateString()) }, [`sb-${REF}-auth-token`, JSON.stringify(session)]);
...
if (TARGET === "admin") {
  await page.waitForFunction(() => document.getElementById("app")?.classList.contains("on"), null, { timeout: 40000 }); await page.waitForTimeout(2500);
  const tabs = await page.evaluate(() => [...document.querySelectorAll(".side .tab[data-view]")].filter((t) => !t.hidden).map((t) => t.dataset.view));
  for (const v of tabs) { await page.evaluate((v) => document.querySelector(`.side .tab[data-view="${v}"]`)?.click(), v); await page.waitForTimeout(v === "gex" || v === "memes" ? 5000 : 1500) }
  console.log("policy:", CSP.slice(0, 100) + "…"); console.log("admin tabs walked:", tabs.length, "| page errors:", errs.length ? errs.join(" | ") : "none");
  if (blocked.size) { console.log("BLOCKED (" + blocked.size + "):"); for (const b of blocked) console.log(" - " + b); await browser.close(); process.exit(1) } else { console.log("NOTHING BLOCKED"); await browser.close(); process.exit(0) }
}
await page.waitForSelector("#td-h1", { timeout: 40000 }); await page.waitForTimeout(2500);
