...
let leaks = 0
for (const [name, path] of [['chat messages', 'messages?select=id&limit=3'], ['feed posts', 'member_recaps?select=id&limit=3'], ['comments', 'post_comments?select=id&limit=3'], ['profiles', 'profiles?select=username&limit=3'], ['library', 'library_videos?select=id&limit=3'], ['video likes', 'library_likes?select=video_id&limit=3'], ['applications', 'applications?select=email&limit=3'], ['channels', 'channels?select=slug&limit=9'], ['follows', 'follows?select=follower_id&limit=3'], ['member_directory', 'member_directory?select=username&limit=3'], ['chat_staff', 'chat_staff?select=user_id&limit=3'], ['mods', 'mods?select=user_id&limit=3'], ['likes', 'post_likes?select=user_id&limit=3'], ['lessons paid', 'lessons?select=id&is_preview=eq.false&limit=3'], ['entitlements', 'entitlements?select=id&limit=3']]) {
