...
for (const name of VPS) {
  ...
  console.log(`${name}: pulse "${m.pulse}" [${m.pulseClass}] live ${m.liveRows} ticks ${m.tickRows} cells ${m.coverageCells} hero ${m.heroSvg ? 'chart' : m.heroEmpty ? 'empty state' : 'NOTHING'} stats ${m.stats.join(' / ')}`)
