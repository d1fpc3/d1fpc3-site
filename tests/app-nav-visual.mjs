...
// ── desktop 1440 ────────────────────────────────────────────────
{
  ...
  if ((await page.getAttribute('#tabs .tab[data-view="chat"]', "aria-current")) !== "page") fails.push("aria-current missing on the lit row");
  if (!/g h/.test(await page.getAttribute('#tabs .tab[data-view="chat"]', "title"))) fails.push("tooltip lacks the jump key");
  if ((await page.getAttribute("#tb-seg", "role")) !== "tablist") fails.push("segment is not a tablist");
  const tbh = await page.$eval(".topbar", (n) => n.getBoundingClientRect().height); if (tbh > 64) fails.push("chat top bar not compact (measured on Chat): " + tbh);
  await page.focus('#tb-seg button[data-view="chat"]'); await page.keyboard.press("ArrowRight"); await page.waitForTimeout(700);
  if (await curView(page) !== "members") fails.push("ArrowRight did not move to Members");
  if (!(await page.$eval("#pane-title", (h) => h.classList.contains("swap")))) fails.push("title did not get the rise class");
  await page.click("#tb-bell"); await page.waitForTimeout(1500);
  if (!(await page.locator("#nbp").isVisible())) fails.push("bell popover did not open");
  if (await curView(page) !== "members") fails.push("bell left the page on desktop");
  const nbpBox = await page.locator("#nbp").boundingBox(), bellBox = await page.locator("#tb-bell").boundingBox();
  if (nbpBox && bellBox && (nbpBox.y < bellBox.y + bellBox.height || nbpBox.x + nbpBox.width > 1440)) fails.push("popover misplaced: " + JSON.stringify(nbpBox));
  if (!(await page.locator("#nbp .nb-row, #nbp .nbp-empty").first().isVisible())) fails.push("popover has no rows and no empty state");
  await shot("07-bell-popover");
  await page.keyboard.press("Escape"); await page.waitForTimeout(400);
  if (await page.locator("#nbp").isVisible()) fails.push("Escape did not close the popover");
  await page.click("#tb-bell"); await page.waitForTimeout(900); await page.click("#nbp-all"); await page.waitForTimeout(700);
  if (await curView(page) !== "inbox") fails.push("See all did not open the inbox");
  ...
  await shot("07b-palette-actions"); await page.keyboard.press("Escape"); await page.waitForTimeout(400);
