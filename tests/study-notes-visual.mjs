...
for (const [name, vp] of [["desk", { viewport: { width: 1440, height: 900 } }], ["phone", { ...devices["iPhone 13"], viewport: { width: 390, height: 844 }, deviceScaleFactor: 2, isMobile: true, hasTouch: true }]]) {
  ...
  const open = async () => { await p.goto("http://127.0.0.1:8123/echelon/app/?start=study", { waitUntil: "domcontentloaded" }); await p.waitForSelector("#toc-list .chap-body button", { state: "attached", timeout: 25000 }); await p.evaluate(() => { document.getElementById("nextup")?.remove(); document.querySelectorAll("#toc-list .chap-body button")[1].click() }); await p.waitForTimeout(700); };
  await open();
