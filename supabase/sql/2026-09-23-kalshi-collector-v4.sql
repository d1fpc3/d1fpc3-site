-- Kalshi collector v4: per-minute features, a third hypothesis family, and the learner's
-- tables. Applied 2026-09-23 on top of v1..v3b. Idempotent.
--
-- D1: "it needs to learn". The learner itself runs in Node (worker/learn.mjs): a logistic
-- model per series, refit nightly on the trailing 30 days, scored out of sample the next
-- day. Two variants: `full` sees the market mid plus spot features; `spot` is blind to
-- the market and sees spot features only, which answers whether the spot feed carries
-- anything the price does not already hold. This file gives it what it reads and where
-- it writes.
--
--   kalshi_features: per (round, minute) from the spot feed: bps from the open, the
--     round's range so far, 3-minute momentum, hour of day. Built per scored day by
--     kalshi_features_build(), called first thing in kalshi_calibrate().
--   'mom' joins 'band' and 'dist' as an edge-hunt key.
--   kalshi_models: the fitted coefficients per (day, series, kind), with training n and
--     Brier, so a reader can see what was learned and from how much.
--   kalshi_daily.brier_model / brier_model_spot: the out-of-sample scores.
--   kalshi_learn_rows: the one view the learner pulls.

-- ─── features ───────────────────────────────────────────────────────────────

create table if not exists public.kalshi_features (
  ticker      text        not null references public.kalshi_rounds (ticker) on delete cascade,
  minute_utc  timestamptz not null,
  mtc         int         not null,
  bps         numeric,               -- spot now vs spot at the round's open, basis points
  range_bps   numeric,               -- (high - low of spot since open) / open, basis points
  mom_bps     numeric,               -- spot now vs spot 3 minutes ago, basis points
  hour_utc    int         not null,
  primary key (ticker, minute_utc)
);
alter table public.kalshi_features enable row level security;
drop policy if exists kalshi_features_admin_read on public.kalshi_features;
create policy kalshi_features_admin_read on public.kalshi_features for select to authenticated using ((select is_admin()));
revoke all on public.kalshi_features from anon, public;
grant select on public.kalshi_features to authenticated;
grant all on public.kalshi_features to service_role;

create or replace function public.kalshi_features_build(p_day date)
returns int language plpgsql security definer set search_path = public as $$
declare v int;
begin
  insert into kalshi_features (ticker, minute_utc, mtc, bps, range_bps, mom_bps, hour_utc)
  select r.ticker, s.minute_utc,
         floor(extract(epoch from (r.close_time - s.minute_utc)) / 60)::int,
         case when so.price > 0 and sp.price is not null then (sp.price - so.price) / so.price * 10000 end,
         case when so.price > 0 and rg.hi is not null then (rg.hi - rg.lo) / so.price * 10000 end,
         case when sm.price > 0 and sp.price is not null then (sp.price - sm.price) / sm.price * 10000 end,
         extract(hour from s.minute_utc at time zone 'utc')::int
  ...
  left join kalshi_spot sm on sm.asset = kalshi_asset(r.series) and sm.minute_utc = s.minute_utc - interval '3 minutes'
  left join lateral (
    select max(z.price) as hi, min(z.price) as lo
    from kalshi_spot z
    where z.asset = kalshi_asset(r.series) and z.minute_utc between r.open_time and s.minute_utc
  ) rg on true
  where (r.close_time at time zone 'utc')::date = p_day
    and s.minute_utc >= r.close_time - interval '15 minutes'
    and s.minute_utc <  r.close_time
  on conflict (ticker, minute_utc) do update set
    mtc = excluded.mtc, bps = excluded.bps, range_bps = excluded.range_bps,
    mom_bps = excluded.mom_bps, hour_utc = excluded.hour_utc;
  get diagnostics v = row_count;
  return v;
end $$;
revoke all on function public.kalshi_features_build(date) from public, anon;
grant execute on function public.kalshi_features_build(date) to service_role;

-- 3-minute momentum buckets, basis points.
create or replace function public.kalshi_mom_bucket(bps numeric)
returns text language sql immutable as $$
  select case
    when bps is null then null
    when bps < -15 then '<-15'
    when bps <  -5 then '-15..-5'
    when bps <   0 then '-5..0'
    when bps <   5 then '0..5'
    when bps <  15 then '5..15'
    else '>15' end
$$;

-- ─── the learner's tables ───────────────────────────────────────────────────

create table if not exists public.kalshi_models (
  as_of        date    not null,          -- the last training day; scores the day after
  series       text    not null,
  kind         text    not null,          -- 'full' (sees the mid) or 'spot' (market-blind)
  coef         jsonb   not null,          -- {feature: weight}, standardized inputs
  n_train      int     not null,
  brier_train  numeric,
  window_days  int     not null,
  primary key (as_of, series, kind, window_days)
);
alter table public.kalshi_models enable row level security;
drop policy if exists kalshi_models_admin_read on public.kalshi_models;
create policy kalshi_models_admin_read on public.kalshi_models for select to authenticated using ((select is_admin()));
revoke all on public.kalshi_models from anon, public;
grant select on public.kalshi_models to authenticated;
grant all on public.kalshi_models to service_role;

alter table public.kalshi_daily add column if not exists brier_model      numeric;
alter table public.kalshi_daily add column if not exists brier_model_spot numeric;
alter table public.kalshi_daily add column if not exists model_n_train    int;

-- Everything the learner needs, one row per scored (round, minute).
create or replace view public.kalshi_learn_rows
with (security_invoker = true) as
select r.series, r.ticker, s.minute_utc,
       (r.close_time at time zone 'utc')::date as day,
       f.mtc, s.mid, s.yes_ask, s.yes_bid, f.bps, f.range_bps, f.mom_bps, f.hour_utc,
       r.settlement_value as y
from public.kalshi_rounds r
join public.kalshi_book_snaps s using (ticker)
join public.kalshi_features f on f.ticker = s.ticker and f.minute_utc = s.minute_utc
where r.result is not null and r.settlement_value is not null and s.mid is not null;
grant select on public.kalshi_learn_rows to authenticated, service_role;
revoke all on public.kalshi_learn_rows from anon, public;
...
-- ─── calibrate: build features first, read bps from them ────────────────────
...
-- The features table needs every scored day that already exists; the backfill's scoring
-- pass builds them as it goes, and this catches days scored before v4.
do $$
declare d record;
begin
  for d in select distinct as_of from kalshi_daily where series = 'ALL' order by 1 loop
    perform public.kalshi_features_build(d.as_of);
  end loop;
end $$;
